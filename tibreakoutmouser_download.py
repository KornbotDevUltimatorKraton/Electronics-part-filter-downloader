...
key_words = " Texas instrument angle magnetic sensor"       #Sensor Magnetic sensor #motor Dc motor BLDC motor driver RF chip 
...
def Getpackage_all_link(dataout,config_data):
      ...
      for ij in range(0,len(list(getpart))): 
                 ...
                 Price_package[list(getpart)[ij].get(Product_info[16]).split("https://")[1].split("/")[len(plen)-1].split("?")[0]] = list(getpart)[ij].get(Product_info[14])
      ...
      print("Price data",Price_package) 
...
def Filter_collection(Pack_drawing_json): 
    for p in list(Pack_drawing_json):
      list_exsist_data = os.listdir(Document) #Checking the document download path       
      text  = p.lower()
      ...
      if keys_data not in list_exsist_data:
         try:
          ...
          print("The file is already downloaded in ",Document)
